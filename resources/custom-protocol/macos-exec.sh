...
if [[ $1 == "lsfusion-protocol://"* ]]; then
  ...
  # macOS ships a native command-line launcher (Contents/MacOS/idea) that supports
  # "--line N <path>" and forwards to the running instance, so run it directly.
  "$IDEA_RUNNABLE" $LINE "$TARGET_PATH"
fi
