...
class DefaultHighlightVisitor implements HighlightVisitor, DumbAware {
  ...
  public void visit(@NotNull PsiElement element) {
    ...
    if (myAnnotationHolder.hasAnnotations()) {
      for (Annotation annotation : myAnnotationHolder) {
        myHolder.add(CustomHighlightInfo.fromAnnotation(annotation, null, myBatchMode));
      }
      myAnnotationHolder.clear();
    }
  }
  ...
  private void runAnnotators(PsiElement element) {
    ...
    List<Annotator> annotators = null;
      try {
        Method method = CachedAnnotators.class.getDeclaredMethod("get", String.class);
        method.setAccessible(true);
        annotators = (List<Annotator>) method.invoke(myCachedAnnotators, element.getLanguage().getID());
    ...
    if (annotators == null || annotators.isEmpty()) return;
