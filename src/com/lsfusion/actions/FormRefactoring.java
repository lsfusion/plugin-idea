...
//        }
        if (element instanceof LSFPrevTopActionPropertyDefinitionBody) {
            LSFPrevTopActionPropertyDefinitionBody statement = (LSFPrevTopActionPropertyDefinitionBody) element;
...
//                    optionsList.getParent().getNode().addChild(newOptionList.getNode(), whitespace);
    ...
    static private void changeCode(MetaTransaction transaction, LSFPrevTopActionPropertyDefinitionBody listDecl) {
        ...
        if(action.getListActionPropertyDefinitionBody() != null) { // ACTION list - просто убираем REMOVE(ACTION)
            ...
            if(nnode != null && LSFParserDefinition.isWhiteSpace(nnode.getElementType())) {
                ...
                listDecl.getNode().removeChild(nnode);
            }
            ...
            listDecl.getNode().replaceChild(node, lbr);
