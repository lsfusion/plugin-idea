...
public class FormRefactoring3 extends com.intellij.openapi.actionSystem.AnAction {
...
//        }
        if (element instanceof LSFOnEditEventSetting) {
            LSFOnEditEventSetting statement = (LSFOnEditEventSetting) element;
            changeCode(transaction, statement);
            return; // дальше нет
        }
        
        if(element instanceof LSFEventStatement) {
            LSFEventStatement statement = (LSFEventStatement) element;
            LSFPrevOrderDo prevOrderDo = statement.getPrevOrderDo();
            if(prevOrderDo != null && !prevOrderDo.getText().isEmpty()) {
                System.out.println(prevOrderDo.getContainingFile().getName() + " " +prevOrderDo.getText());
            }                
        }            
...
//                    optionsList.getParent().getNode().addChild(newOptionList.getNode(), whitespace);
    ...
    static private void changeCode(MetaTransaction transaction, LSFOnEditEventSetting listDecl) {

        // в одну строку ACTION f(a); -> REPLACE({) f(a);ADD( }), 
        //              ACTION f(a) ... pOptions ... ; -> REPLACE({) f(a)ADD(; }) ... ;

        LSFActionPropertyDefinitionBody action = listDecl.getActionPropertyDefinitionBody();
        if(action.getListActionPropertyDefinitionBody() != null) { // ACTION list - просто убираем REMOVE(ACTION)
            ...
            if(!action.getText().contains("{")) { // одна строка
                ...
                addTokens(BaseUtils.toList(lbr, ASTFactory.whitespace(" ")), transaction, action.getNode(), true);
