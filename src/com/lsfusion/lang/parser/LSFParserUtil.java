...
public class LSFParserUtil extends GeneratedParserUtilBase {
    ...
    private static Key<Integer> MATCHEDCLASS = Key.create("lsf.matched.class");

    public static boolean checkClassOrExpression(PsiBuilder builder_, int level_) {
        builder_.putUserData(MATCHEDCLASS, null);
        return true;
    }
    public static boolean matchedClass(PsiBuilder builder_, int level_) {
        builder_.putUserData(MATCHEDCLASS, builder_.rawTokenIndex());
        return true;
    }

    public static boolean matchedClassOrExpression(PsiBuilder builder_, int level_) {
        Integer userData = builder_.getUserData(MATCHEDCLASS);
        if (userData != null && userData == builder_.rawTokenIndex()) {
            builder_.putUserData(MATCHEDCLASS, null); // just in case
            return false;
        }        
        return true;
    }
...
//    }
    ...
    private static Key<Boolean> EXTEND_OBJECTS_KEY = Key.create("lsf.extend.objects");
    public static boolean setExtendObjects(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_OBJECTS_KEY, true);
        return true;
    }
    public static boolean resetExtendObjects(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_OBJECTS_KEY, false);
        return true;
    }
    public static boolean isExtendObjects(PsiBuilder builder_, int level_) {
        Boolean value = builder_.getUserData(EXTEND_OBJECTS_KEY);
        return value != null && value;
    }

    private static Key<Boolean> EXTEND_TREE_KEY = Key.create("lsf.extend.tree");
    public static boolean setExtendTree(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_TREE_KEY, true);
        return true;
    }
    public static boolean resetExtendTree(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_TREE_KEY, false);
        return true;
    }
    public static boolean isExtendTree(PsiBuilder builder_, int level_) {
        Boolean value = builder_.getUserData(EXTEND_TREE_KEY);
        return value != null && value;
    }

    private static Key<Boolean> EXTEND_PROPERTIES_KEY = Key.create("lsf.extend.properties");
    public static boolean setExtendProperties(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_PROPERTIES_KEY, true);
        return true;
    }
    public static boolean resetExtendProperties(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_PROPERTIES_KEY, false);
        return true;
    }
    public static boolean isExtendProperties(PsiBuilder builder_, int level_) {
        Boolean value = builder_.getUserData(EXTEND_PROPERTIES_KEY);
        return value != null && value;
    }

    private static Key<Boolean> EXTEND_FILTERS_KEY = Key.create("lsf.extend.filters");
    public static boolean setExtendFilters(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_FILTERS_KEY, true);
        return true;
    }
    public static boolean resetExtendFilters(PsiBuilder builder_, int level_) {
        builder_.putUserData(EXTEND_FILTERS_KEY, false);
        return true;
    }
    public static boolean isExtendFilters(PsiBuilder builder_, int level_) {
        Boolean value = builder_.getUserData(EXTEND_FILTERS_KEY);
        return value != null && value;
    }
}